...
class MyStack extends TerraformStack {
  ...
  constructor(scope: Construct, id: string, config: MyStackConfig) {
    ...
    if (!config.id) {
      this.instance = new EC2.Instance(this, "Hello", {
        ami: "ami-2757f631",
        instanceType: "t2.micro",
      });
    }

    new EC2.Eip(this, "World", {
      instance: config.id ?? this.instance.id,
    });
  }
}

const app = new App();
const origin = new MyStack(app, "origin-stack", {});
new MyStack(app, "target-stack", { id: origin.instance?.id });

app.synth();
```

From a usage perspective it looks like we are accessing the value of `instance` from the `origin-stack` and then referencing it in the `target-stack`.
