...
function terraformCloudConfig(baseName, organizationName, workspaceName) {
  ...
  result = template.replace(`new LocalBackend(this);`, `new RemoteBackend(this, new RemoteBackendProps { Hostname = "app.terraform.io", Organization = "${organizationName}", Workspaces = new NamedRemoteWorkspace("${workspaceName}") });`);
